...
import com.qdong.communal.library.module.network.custom_gson_parser.GsonConverterFactory2;
...
public class RetrofitAPIManager {
    ...
    private static final long DEFAULT_CONNECT_TIMEOUT = 10_000;//连接时间,10秒
    private static final long DEFAULT_WRITE_TIMEOUT =300_000 ;//写,300秒
    private static final long DEFAULT_READ_TIMEOUT = 300_000;//度,300秒
    ...
    /**
     * @method name:provideClientApi
     * @des:获取client,不再自动解析json
     * @param :[context, url:baseUrl, isFileUpload:是否是文件上传]
     * @return type:com.qdong.communal.library.module.network.QDongApi
     * @date 创建时间:2016/8/25
     * @author Chuck
     **/
    public static QDongApi provideClientApiNoParse(Context context,String url,boolean isFileUpload) {

        JSESSIONID= SharedPreferencesUtil.getInstance(context).getString(Constants.SESSION_ID,"-1");
        Retrofit retrofit = getRetrofitNoParse(url, isFileUpload);
        return retrofit.create(QDongApi.class);
    }
    ...
    private static Retrofit getRetrofit(String url, boolean isFileUpload) {
        return new Retrofit.Builder()
                    ...
                    //.addConverterFactory(GsonConverterFactory2.create())//json解析器
    ...
    @NonNull
    private static Retrofit getRetrofitNoParse(String url, boolean isFileUpload) {
        return new Retrofit.Builder()
                    .client(isFileUpload?genericFileUploadClient():genericClient())
                    .addCallAdapterFactory(RxJavaCallAdapterFactory.create())//调用适配器
                    .baseUrl(TextUtils.isEmpty(url)?Constants.SERVER_URL:url)
