...
/**
 * 原来的StarEvaluateView不能在DataBind里使用,因为原来的StarEvaluateView里的ratBar用了setTag,
 * 所以,这个类里面的ratBar使用自定义的,加一个属性,取代原来的setTag
 *
 *
 * 星级评分控件,是垂直的线性布局.实现多个项目打分.每行一排星星
 * 通过xml  findViewById实例化,
 * 通过{@link StarView4DataBinding#setItemNames(String...)}设置每个条目名,同时开始展示;
 * 通过{@link StarView4DataBinding#setDefaultChosen(float ,String...)}设置每个条目默认选中几颗星,同时开始展示;
 * 通过{@link StarView4DataBinding#getResult()}获取结果
 * 
 * 
 * StarEvaluateView.java
 * 责任人:  Chuck
 ...
 */
public class StarView4DataBinding extends LinearLayout implements OnRatingBarChangeListener {
	 ...
	 */
	@SuppressLint("NewApi")
	public StarView4DataBinding(Context context, AttributeSet attrs,
	 ...
	 * @param defStyle
	 */
	public StarView4DataBinding(Context context, AttributeSet attrs, int defStyle) {
	...
	private void initView() {
		...
		for(int i=0;i<linesCount;i++){
			View view =null;
			view=inflater.inflate(isBig?R.layout.include_star_evaluate_item_big_4_databinding:R.layout.include_star_evaluate_item_small_4_databinding, null);
			CustomRatingBar ratingBar = (CustomRatingBar) view.findViewById(R.id.ratingBar_rating_average);
	...
	public void onRatingChanged(RatingBar ratingBar1, float rating,
								boolean fromUser) {
		CustomRatingBar ratingBar= (CustomRatingBar) ratingBar1;
		Integer index  = (Integer) ratingBar.getmTag();
